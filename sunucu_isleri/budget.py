...
# 3. TEK BİR PROJE GETİRME (YENİ!)
@router.get("/get/{item_id}")
async def get_budget_project(item_id: int):
    try:
        conn = get_db_connection()
        cursor = conn.cursor(dictionary=True)
        cursor.execute("SELECT * FROM budget_projects WHERE id = %s", (item_id,))
        item = cursor.fetchone()
        cursor.close()
        conn.close()
        if not item:
            raise HTTPException(status_code=404, detail="Budget projesi bulunamadı")
        return item
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_budget_project(item_id: int, project: ProjectCreate):
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        sql = """UPDATE budget_projects SET 
